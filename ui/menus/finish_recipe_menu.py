...
class FinishRecipeMenu:
    ...
    def display_finish_recipe_menu() -> None:
        ...
        Styler.print_gray("en voer dit in. Weeg daarna je portie af en voer dit in.")
        Styler.print_gray("Brandy berekent nu het aantal calorieën in je portie.")
        Styler.print_gray("Indien het gaat om (bijvoorbeeld) een salade waar het totale")
        Styler.print_gray("gewicht na bereiding niet veel varieert, kun je het eindaantal")
        Styler.print_gray("laten berekenen op basis van de gewichten van de ingevulde")
        Styler.print_gray("ingrediënten.")
        Styler.print_blue("--={*}=--\n")
